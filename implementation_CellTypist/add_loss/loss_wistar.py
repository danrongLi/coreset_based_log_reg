...
logger.info("afterward shape is:")
logger.info(selected_ada.shape)
...
start = time.time()
my_model = celltypist.train(adata_train, "leiden1p2_labels",alpha=1/(0.001*len(adata_train)), feature_selection=False, check_expression = True,use_SGD = True,mini_batch = True, batch_size=batch_size, batch_number=100)
end=time.time()
...
true_labels = adata_test.obs['leiden1p2_labels']  # Assuming true labels are stored here
...
file_name = f'/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/reports/classification_report_wistar_celltypist.csv'
...
predicted_labels = predictions.predicted_labels.values  # Array of predicted labels from AnnotationResult
probability_matrix = predictions.probability_matrix.values  # Probability matrix
true_labels = adata_train.obs['leiden1p2_labels']  # Assuming true labels are stored here
