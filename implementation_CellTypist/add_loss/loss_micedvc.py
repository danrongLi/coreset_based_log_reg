...
start = time.time()
my_model = celltypist.train(adata_train, "identity_layer2",alpha=1/(0.01*len(adata_train)), feature_selection=False, check_expression = True,use_SGD = True,mini_batch = True, batch_size=batch_size, batch_number=100)
end=time.time()
...
true_labels = adata_test.obs['identity_layer2']  # Assuming true labels are stored here
...
file_name = f'/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/reports/classification_report_micedvc_celltypist.csv'
...
predicted_labels = predictions.predicted_labels.values  # Array of predicted labels from AnnotationResult
probability_matrix = predictions.probability_matrix.values  # Probability matrix
true_labels = adata_train.obs['identity_layer2']  # Assuming true labels are stored here
