...
start = time.time()
my_model = celltypist.train(adata_train, "cell_type",alpha=1/(0.01*len(adata_train)), check_expression = True, feature_selection=False,use_SGD = True,mini_batch = True,C=0.1, batch_size=batch_size, batch_number=100)
end=time.time()
...
true_labels = adata_test.obs['cell_type']  # Assuming true labels are stored here
...
file_name = f'/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/reports/classification_report_simonson_celltypist.csv'
# Save the DataFrame as a CSV file
report_df.to_csv(file_name, index=True)
logger.info("done saving the classification report")


logger.info("lets find the log loss for train data")
predictions = celltypist.annotate(adata_train, model = my_model,majority_voting = False,mode = 'best match')
predicted_labels = predictions.predicted_labels.values  # Array of predicted labels from AnnotationResult
probability_matrix = predictions.probability_matrix.values  # Probability matrix
true_labels = adata_train.obs['cell_type']  # Assuming true labels are stored here
