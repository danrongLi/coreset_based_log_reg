...
for C in C_list:
    ...
    #    adata_test_fold.obs['identity_layer2'] = y_test
        ...
        start = time.time()
        my_model = celltypist.train(adata_train_fold, "identity_layer2",alpha=1/(C*len(adata_train_fold)), feature_selection=False, check_expression = True,use_SGD = True,mini_batch = True, batch_size=batch_size, batch_number=100)
        ...
        true_labels = adata_test_fold.obs['identity_layer2']  # Assuming true labels are stored here
...
#y axis: true label. x axis: predicted
        ...
        predicted_labels = predictions.predicted_labels.values  # Array of predicted labels from AnnotationResult
        probability_matrix = predictions.probability_matrix.values  # Probability matrix
        true_labels = adata_train_fold.obs['identity_layer2']  # Assuming true labels are stored here
