...
coreset_sizes = [10000,20000,30000,40000,50000]

for size in coreset_sizes:
    logger.info(size)
    # selected_ada = anndata.read("/storage/home/dvl5760/scratch/heca_200k.h5ad")
    selected_ada = anndata.read("/Users/danrongli/Desktop/Feature_Space_Logistic/server_results/oct23/heca_200k.h5ad")
    if size == selected_ada.shape[0]:
        selected_ada = selected_ada
    else:
        selected_ada = celltypist.samples.downsample_adata(adata = selected_ada, mode = "total",n_cells = size, by = "cell_type",random_state=42,return_index=False)
    ...
    current = time.time()
    svd = TruncatedSVD(n_components=250, algorithm='randomized', random_state=42)
