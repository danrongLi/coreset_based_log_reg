...
def not_train_logistic_regression_withoutCV(X_train, y_train):
    """
    Train logistic regression model with multi-class 'one-vs-rest' strategy.
    """
    logger.info("begin training")
...
def not_train_logistic_regression(X_train, y_train):
    """
    Train logistic regression model with multi-class 'one-vs-rest' strategy and use
    cross-validation to find the best C parameter.
    """
    logger.info("begin training with cross-validation")

    # Define the parameter grid for C
    #param_grid = {'C': np.logspace(-4, 2, 10)}  # C values from 1e-4 to 1e2
    param_grid = {'C': np.logspace(-3, 1, 5)}  # Smaller range
    # Initialize the logistic regression model
    #clf = LogisticRegression(multi_class='ovr', solver='saga', max_iter=200, n_jobs=-1)
    clf = LogisticRegression(multi_class='ovr', solver='lbfgs', max_iter=200, n_jobs=-1)
    ...
    logger.info("end doing the gridsearch")
    time_taken = end-begin
    logger.info(f"Time taken for gridsearch: {time_taken:.4f} seconds")
    # Fit the model using GridSearchCV
    begin = time.time()
    if hasattr(X_train, "toarray"):  # Check if X_train is sparse
        logger.info("X_train is sparse")
        #X_train = X_train.toarray()
        logger.info("keeping it sparse for fitting")
    #X_train = csr_matrix(X_train)
    ...
    logger.info("end fitting the best_clf")
    time_taken = end-begin
    logger.info(f"Time taken for training: {time_taken:.4f} seconds")
    # Best found model
    best_clf = grid_search.best_estimator_
    best_C = grid_search.best_params_['C']
    logger.info(f"Best C parameter found: {best_C}")
    return best_clf
...
def edit_predict_with_confidence(model, X_test_scaled, lr_idx, mode='best match', p_thres=0.5):
...
def run_pipeline(selected_ada, X_train,X_train_entire, y_train,y_train_entire, X_test,y_test, gene_names,model_features, resolution=1.0, min_prop=0.0):
    ...
    X_train_filtered_scaled = scaler.fit_transform(X_train.toarray()).astype(float)
    X_train_entire_filtered_scaled = scaler.transform(X_train_entire.toarray()).astype(float)
    ...
    y_pred, decision_matrix, probability_matrix = edit_predict_with_confidence(clf, X_test_scaled, gene_indices)
    ...
    _, _, prob_matrix_train_sampled = predict_with_confidence(clf, X_train_filtered_scaled)
    ...
    X_train_size = round(len(y_train)/len(y_train_entire),1)
    
    file_name = f'/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/reports/classification_report_simonson_size_{X_train_size}.csv'
    ...
    _, _, prob_matrix_train_entire = predict_with_confidence(clf, X_train_entire_filtered_scaled)
...
cell_type_counts = selected_ada.obs['cell_type'].value_counts()
valid_cell_types = cell_type_counts[cell_type_counts >= 2].index
selected_ada = selected_ada[selected_ada.obs['cell_type'].isin(valid_cell_types)].copy()


cell_type_counts = selected_ada.obs['cell_type'].value_counts()
plt.figure(figsize=(10, 6))
cell_type_counts.plot(kind='bar', color='skyblue')
plt.title("Cell Type Distribution in simonson Data")
plt.xlabel("Cell Type")
plt.ylabel("Count")
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
plt.savefig("/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/pdf/cell_type_distribution_simonson_entire.pdf", format="pdf")
...
for train_indices, test_indices in splitter.split(selected_ada.X, selected_ada.obs['cell_type']):
    adata_train = selected_ada[train_indices].copy()
    adata_test = selected_ada[test_indices].copy()
...
#if hasattr(X_test, "toarray"):  # Check if X_train is sparse
#    logger.info("X_test is sparse")
...
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
plt.savefig("/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/pdf/cell_type_distribution_simonson_train.pdf", format="pdf")
...
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
plt.savefig("/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/pdf/cell_type_distribution_simonson_test.pdf", format="pdf")
...
if not isinstance(X_train, csr_matrix):
    X_train = csr_matrix(X_train)
    logger.info("X_train is now csr")
if not isinstance(X_test, csr_matrix):
    X_test = csr_matrix(X_test)
    logger.info("X_test is now csr")

#if issparse(sampled_X_train):  # Convert only if it's sparse
#    sampled_X_train = sampled_X_train.toarray()

#if issparse(sampled_X_test):  # Convert only if it's sparse
#    sampled_X_test = sampled_X_test.toarray()
...
#    logger.info("X_train is sparse")
#    X_train = X_train.toarray()
#    logger.info("X_train is now dense")

#if hasattr(X_test, "toarray"):  # Check if X_train is sparse
#    logger.info("X_test is sparse")
#    X_test = X_test.toarray()
#    logger.info("X_test is now dense")


#coreset_ratio = [0.1,0.11,0.13,0.15,0.17,0.19,0.21,0.23,0.25,0.27,0.29,0.3,0.31,0.33,0.35,0.37,0.39,0.5,0.7,0.9,1.0]
...
for size in coreset_size:
    ...
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    plt.savefig(f"/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/pdf/cell_type_distribution_simonson_train_sampled_{size}.pdf", format="pdf")
