...
def not_train_logistic_regression_without_cv(X_train, y_train):
    """
    Train logistic regression model with multi-class 'one-vs-rest' strategy.
    """
    logger.info("begin training")
...
def not_train_logistic_regression(X_train, y_train):
    ...
    grid_search = RandomizedSearchCV(clf, param_distributions=param_grid,
                                   n_iter=5, cv=3, scoring='neg_log_loss',
...
def run_pipeline(selected_ada, X_train,X_train_entire, y_train,y_train_entire, X_test,y_test, gene_names,model_features, resolution=1.0, min_prop=0.0):
    ...
    file_name = f'/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/reports/classification_report_micedvc_size_{X_train_size}.csv'
    ...
    cm_file = (
        ...
        f"test_train_split/pdf/confusion_matrix_micedvc_size_{X_train_size}.pdf"
    )
    #plt.savefig(cm_file)
    plt.close()
...
selected_ada = selected_ada[~selected_ada.obs['identity_layer2'].isna()]
...
plt.tight_layout()
#plt.savefig("/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/pdf/cell_type_distribution_micedvc_entire.pdf", format="pdf")
...
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
#plt.savefig("/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/pdf/cell_type_distribution_micedvc_train.pdf", format="pdf")
...
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
#plt.savefig("/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/pdf/cell_type_distribution_micedvc_test.pdf", format="pdf")
...
for size in coreset_size:
    ...
    start = time.time()
    sampled_indices = celltypist.samples.downsample_adata(adata=adata_train, mode="each",n_cells=num_per_cell_type, by="identity_layer2", random_state=42, return_index=True)
    ...
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    #plt.savefig(f"/storage/home/dvl5760/work/our_log_reg/change_coreset/add_loss/test_train_split/add_crossval_fix_cellnum/pdf/cell_type_distribution_micedvc_train_sampled_{size}.pdf", format="pdf")
